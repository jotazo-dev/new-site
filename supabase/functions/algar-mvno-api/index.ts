...
serve(async (req) => {
  ...
  try {
    ...
    if (user) {
      ...
      if (!slug) {
        console.error(`[Algar] User ${user.id} has no role`);
    ...
    let { clientId, clientSecret, baseUrl, method, path, body, environment, forceMock, queryParams } = await req.json();

    // Fallback: load Algar credentials server-side (bypassing client RLS) so non-admin
    // collaborators with section permission can still call this function.
    if (!clientId || !clientSecret || !baseUrl) {
      const adminDb2 = createClient(
        Deno.env.get("SUPABASE_URL") ?? "",
        Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") ?? ""
      );
      const { data: integ } = await adminDb2
        .from("integrations")
        .select("config")
        .eq("provider", "algar")
        .maybeSingle();
    ...
    // Final fallback: project secrets (works even when integrations row is missing).
    const envIsSandbox = (environment || "").toLowerCase() === "sandbox";
    clientId = clientId || (envIsSandbox
      ? Deno.env.get("ALGAR_CLIENT_ID_SANDBOX")
      : Deno.env.get("ALGAR_CLIENT_ID_PROD")) || Deno.env.get("ALGAR_CLIENT_ID_PROD");
    clientSecret = clientSecret || (envIsSandbox
      ? Deno.env.get("ALGAR_CLIENT_SECRET_SANDBOX")
      : Deno.env.get("ALGAR_CLIENT_SECRET_PROD")) || Deno.env.get("ALGAR_CLIENT_SECRET_PROD");
    baseUrl = baseUrl || "https://api.onmultitelco.com";
    environment = environment || "production";


    console.log(`[Algar] Environment: ${environment}, Path: ${path}, ForceMock: ${forceMock}, QueryParams:`, queryParams);
    ...
    const targetUrl = `${baseUrl.replace(/\/$/, "")}${path.startsWith("/") ? path : `/${path}`}${queryString}`;
    const reqBodyStr = method !== "GET" && body ? JSON.stringify(body) : "";
    console.log(`[Algar][REQ] ${method} ${targetUrl}`);
    if (reqBodyStr) console.log(`[Algar][REQ] body: ${reqBodyStr.slice(0, 2000)}`);
    ...
    try {
      ...
      console.log(`[Algar][RES] ${apiResponse.status} in ${endTime - startTime}ms: ${apiResponseText.slice(0, 4000)}`);
      if (!apiResponse.ok) {
        console.error(`[Algar][ERR] ${method} ${targetUrl} -> ${apiResponse.status}: ${apiResponseText.slice(0, 4000)}`);
      }
      ...
      return new Response(JSON.stringify({
        ...
        url: targetUrl,
        raw: apiResponseText // Sending raw response back for frontend debugging
      }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 200,
      });
