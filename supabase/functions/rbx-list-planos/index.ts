// Lista planos cadastrados no RBX via serviço v1 ConsultaPlanos.
// Admin-only. Read-only. POST { filtro?: string } → { ok, count, planos, raw }
...
function sanitizeFiltro(input: unknown): string {
  ...
  let s = input.trim();
  if (!s) return "";
  // Bloqueia múltiplas statements e comentários SQL
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!ok) {
      ...
      // erro_code 1 = "A consulta nao retornou resultados" → trate como ok com lista vazia
      if (code === 1) {
        return new Response(
          JSON.stringify({ ok: true, count: 0, planos: [], filtro, latency_ms, message: "Nenhum plano retornado." }),
          { headers: corsHeaders, status: 200 },
        );
      }
      ...
      return new Response(
        JSON.stringify({
          ...
          code,
          message: desc || "Falha ao consultar planos no RBX.",
          latency_ms,
        }),
        { headers: corsHeaders, status: 200 },
      );
    ...
    const planos = extractList(parsed?.result ?? parsed);
    return new Response(
      JSON.stringify({ ok: true, count: planos.length, planos, filtro, latency_ms }),
      { headers: corsHeaders, status: 200 },
    );
  } catch (e) {
    console.error("[rbx-list-planos] exception:", (e as Error).message);
    return new Response(
